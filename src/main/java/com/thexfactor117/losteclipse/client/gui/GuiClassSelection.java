	...
	protected void actionPerformed(GuiButton button) throws IOException
	{
		...
		if (player != null && playerInfo != null)
		{
			if (button == warrior) 
			{
				...
				LostEclipse.network.sendToServer(new PacketClassSelection(1));
			}
			else if (button == mage) 
			{
				playerInfo.setPlayerClass(2);
				LostEclipse.network.sendToServer(new PacketClassSelection(2));
			}
			else if (button == hunter) 
			{
				playerInfo.setPlayerClass(3);
				LostEclipse.network.sendToServer(new PacketClassSelection(3));
			}
